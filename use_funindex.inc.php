   ...
   // header("Location: ../robotweb/index.php?error=emptyfields&uid=");
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      header("Location: ../lunchweb/robotweb/index.php?error=sqlerror");
        ...
        mysqli_stmt_bind_param($stmt, "sss", $teamnumber,$region,$matchnumber);    
        mysqli_stmt_execute($stmt);     
        mysqli_stmt_store_result($stmt);
        $resultCount = mysqli_stmt_num_rows($stmt);
        if($resultCount==0){
        ...
        if (!mysqli_stmt_prepare($stmt2, $sql11)) {
          ...
          mysqli_stmt_bind_param($stmt2, "issiissiiiiiisiiiis", $matchnumber, $teamnumber, $alliance,$numberOfHatch,$numberOfCargo,$autopoints,$platformlvl,$cargolvl1,$cargolvl2,$cargolvl3,$hatchlvl1,$hatchlvl2,$hatchlvl3,$region,$cargoship,$hatchship,$autohatch,$autocargo,$notes);
...
echo '
 ...
 if(isset($_POST["addTeam"])) {
  require 'addTeam.inc.php';
 }

 if(isset($_POST["addMatch"])) {
  require 'addMatch.inc.php';
