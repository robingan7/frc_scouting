...
if (isset($_POST['signup-submit'])) {
  ...
  if (empty($username) || empty($password) || empty($passwordRepeat)) {
    header("Location: ../robotweb/teamMobile.php?error=emptyfields&uid=".$username." password=".$password." con: ".$passwordRepeat);
    exit();
  }

  else if (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {
    header("Location: ../robotweb/teamMobile.php?error=invaliduid&mail=".$email);
    exit();
  }
  
  else if (!isset($_POST['checkbox'])) {
    header("Location: ../robotweb/teamMobile.php?error=notcheck");
    exit();
  }
  
  else if ($password !== $passwordRepeat) {
    header("Location: ../robotweb/teamMobile.php?error=passwordcheck&uid=".$username."&mail=".$email."password=".$password." ".$passwordRepeat);
    exit();
  }
  else {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($resultCount > 0) {
        header("Location: ../robotweb/teamMobile.php?error=usertaken&mail=");
        exit();
      }
      else {
  ...
  header("Location: ../robotweb/teamMobile.php");
  exit();
}